...
void intTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	checkPassFail(intScan(&index,4000, GT,6000, LT), 998) // Test for going beyond the tree
}
...
void errorTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	std::cout << "Search for a wrong key" << std::endl;
        try
         {
	         index.startScan(&int12, GTE, &int20, LTE);
                 std::cout << "Key not found exception test failed" << std::endl;
         }
         catch(NoSuchKeyFoundException e)
         {
                 std::cout << "Key Not found Exception Test 1 Passed." << std::endl;
//		 index.endScan();
         }
	std::cout << "Search completed case" << std::endl;
        try
         {
                 index.startScan(&int9, GTE, &int10, LTE);
		 index.scanNext(scanId); // Return 9
		 index.scanNext(scanId); // must throw an exception  - end of the tree
                 std::cout << "Index scan completed exception test failed" << std::endl;
         }
         catch(IndexScanCompletedException e)
         {
                 std::cout << "Index Scan completed Exception Test 1 Passed." << std::endl;
         }
	index.endScan();
	deleteRelation();
}
...
void deleteRelation()
{
	...
	{
		File::remove(relationName);
		std::cout<< "Relation file removed"<<std::endl;
	}
	catch(FileNotFoundException e)
	{
	}
